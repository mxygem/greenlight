...
func generateToken(userID int64, ttl time.Duration, scope string) (*Token, error) {
	...
	// bytes from your operating system's CSPRNG. This will return an error if the CSPRNG fails to
	// function correctly.
	...
	// field. This will be the token string that we send to the user in their welcome email. They
	// will look similar to this:
	//
	// Y3QMGX3PJ3WLRL2YRTQGQ6KRHU
	//
	// Note that by default base-32 strings may be padded at the end with the = character. We don't
	// need this padding character for the purpose of our tokens, so we use the
	...
	// in the `hash` field of our database table. Note that the sha256.Sum256() function returns an
	// *array* of length 32, so to make it easier to work with we convert it to a slice using the
	// [:] operator before storing it.
